...
static void hw_config_extended_sco_i2spcm_config_cback(void *p_mem)
{
    ...
    if (status == BT_VND_OP_RESULT_SUCCESS)
    {
        if ((opcode == HCI_VSC_WRITE_I2SPCM_INTERFACE_PARAM) && (SCO_INTERFACE_PCM == sco_bus_interface))
        {
            /* Ask a new buffer to hold WRITE_SCO_PCM_INT_PARAM command */
            if (bt_vendor_cbacks)
            {
                p_buf = (HC_BT_HDR *)bt_vendor_cbacks->alloc(BT_HC_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE + SCO_PCM_PARAM_SIZE);
            }
            ...
            if (p_buf)
            {
                ...
                *p++ = SCO_PCM_PARAM_SIZE;
                memcpy(p, &bt_sco_param, SCO_PCM_PARAM_SIZE);
                ALOGI("%s: SCO PCM configure {0x%x, 0x%x, 0x%x, 0x%x, 0x%x}", __FUNCTION__, bt_sco_param[0], bt_sco_param[1], bt_sco_param[2], bt_sco_param[3], bt_sco_param[4]);

                uint8_t ret = bt_vendor_cbacks->xmit_cb(HCI_VSC_WRITE_SCO_PCM_INT_PARAM, p_buf, hw_config_extended_sco_i2spcm_config_cback);
                if (ret == FALSE)
                {
                    bt_vendor_cbacks->dealloc(p_buf);
                }
        ...
        else if ((opcode == HCI_VSC_WRITE_SCO_PCM_INT_PARAM) && (SCO_INTERFACE_PCM == sco_bus_interface))
        {
            /* Ask a new buffer to hold WRITE_PCM_DATA_FORMAT_PARAM command */
            if (bt_vendor_cbacks)
            {
                p_buf = (HC_BT_HDR *)bt_vendor_cbacks->alloc(BT_HC_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE + PCM_DATA_FORMAT_PARAM_SIZE);
            }
            ...
            if (p_buf)
            {
                ...
                *p++ = PCM_DATA_FORMAT_PARAM_SIZE;
                memcpy(p, &bt_pcm_data_fmt_param, PCM_DATA_FORMAT_PARAM_SIZE);
                ALOGI("%s: SCO PCM data format {0x%x, 0x%x, 0x%x, 0x%x, 0x%x}", __FUNCTION__, bt_pcm_data_fmt_param[0], bt_pcm_data_fmt_param[1], bt_pcm_data_fmt_param[2], bt_pcm_data_fmt_param[3], bt_pcm_data_fmt_param[4]);

                uint8_t ret = bt_vendor_cbacks->xmit_cb(HCI_VSC_WRITE_PCM_DATA_FORMAT_PARAM, p_buf, hw_config_extended_sco_i2spcm_config_cback);
                if (ret == FALSE)
                {
                    bt_vendor_cbacks->dealloc(p_buf);
                }
...
static void hw_config_extended_sco_i2spcm_config(uint16_t codec)
{
    ...
    if (p_buf)
    {
        ...
        *p++ = SCO_I2SPCM_PARAM_SIZE;
        ...
        memcpy(p, &bt_sco_i2spcm_param, SCO_I2SPCM_PARAM_SIZE);
        cmd_u16 = HCI_VSC_WRITE_I2SPCM_INTERFACE_PARAM;
        ALOGI("%s: I2SPCM config {0x%x, 0x%x, 0x%x, 0x%x}", __FUNCTION__, bt_sco_i2spcm_param[0], bt_sco_i2spcm_param[1], bt_sco_i2spcm_param[2], bt_sco_i2spcm_param[3]);

        uint8_t ret = bt_vendor_cbacks->xmit_cb(cmd_u16, p_buf, hw_config_extended_sco_i2spcm_config_cback);
        if (ret == FALSE)
        {
            bt_vendor_cbacks->dealloc(p_buf);
        }
...
static void hw_config_extended_set_SCO_codec(uint16_t codec)
{
    ...
    if (bt_vendor_cbacks)
    {
        p_buf = (HC_BT_HDR *)bt_vendor_cbacks->alloc(BT_HC_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE + SCO_CODEC_PARAM_SIZE);
    }
    ...
    if (p_buf)
    {
        ...
        uint8_t ret = bt_vendor_cbacks->xmit_cb(HCI_VSC_ENABLE_WBS, p_buf, p_set_SCO_codec_cback);
        if (ret == FALSE)
        {
            bt_vendor_cbacks->dealloc(p_buf);
        }
...
static void hw_config_extended_sco_config(void)
{
    /*
     Copied from the 'hw_sco_config' method.
     We need to setup all required stucts before SCO configuration.
     */

    ALOGI("%s: set SCO related hardware settings", __FUNCTION__);
...
static void hw_config_extended_start(void)
{
    ...
    hw_config_extended_sco_config();
}

#endif // (BTHW_FW_EXTENDED_CONFIGURATION)
